...
import { ChevronDown, ChevronUp, PlayCircle, Clock, Users, BookOpen, Star, FileText } from 'lucide-react'; 
...
const CourseDetailPage = () => {
    ...
    const courseInfo = {
        ...
        oldPrice: "$69.0",
        newPrice: "$0 (Free)",
        description: "Master the foundations of programming with Python’s simple syntax. Learn variables, loops, functions, and problem-solving skills through hands-on challenges built for beginners and aspiring developers.",
        instructor: {
            name: "John Doe",
        ...
        curriculum: [
            { 
                ...
                items: [
                    { title: "Lesson 1: Introduction to Python", duration: "12:30", type: "Video", preview: true },
                    { title: "Lesson 2: Variables and Data Types", duration: "10:05", type: "Video", preview: true },
                    { title: "Lesson 3: Functions and Loops", duration: "22:25", type: "Video", preview: true },
                ]
            },
            { 
                ...
                items: [
                    { title: "Challenge 1: Basic Math Operations", duration: "10:00", type: "Quiz", preview: false },
                    { title: "Challenge 2: Conditional Logic", duration: "20:00", type: "Quiz", preview: false },
                ]
            },
            { 
                ...
                items: [
                    { title: "Project 1: Simple Web Scraper", duration: "30:00", type: "Video", preview: false },
                    { title: "Project 2: File Handling", duration: "30:00", type: "Video", preview: false },
                ]
            },
        ]
    };
    ...
    const renderIcon = (type) => {
        if (type === 'Video') return <PlayCircle size={18} className="text-[#FFD200]" />;
        if (type === 'Quiz') return <FileText size={18} className="text-[#FFD200]" />;
    ...
    return (
        <div className="bg-[#001433] text-white min-h-screen">
            ...
            <section className="flex justify-between items-start p-12 space-x-10">
                ...
                <div className="text-left w-3/5">
                    ...
                    <div className="flex items-center space-x-6 text-lg font-semibold text-[#A9A9A9] mb-6">
                        <span className="flex items-center space-x-2"><Clock size={18} className="text-[#FFD200]" /> <span>{courseInfo.duration}</span></span>
                        <span className="flex items-center space-x-2"><Users size={18} className="text-[#FFD200]" /> <span>{courseInfo.students}</span></span>
                        <span className="flex items-center space-x-2"><Star size={18} className="text-[#FFD200]" /> <span>{courseInfo.level}</span></span>
                        <span className="flex items-center space-x-2"><BookOpen size={18} className="text-[#FFD200]" /> <span>{courseInfo.lessons}</span></span>
                        <span className="flex items-center space-x-2"><FileText size={18} className="text-[#FFD200]" /> <span>{courseInfo.quizzes}</span></span>
                    </div>

                    {/* Deskripsi */}
                    <p className="text-[16px] font-regular text-white mt-4">{courseInfo.description}</p>
                </div>

                {/* 2.4 Course Preview Card (Kanan) */}
                <div className="w-1/3 flex-shrink-0"> {/* ✅ Penyesuaian lebar kartu */}
                    <div className="bg-[#0B1735] rounded-[20px] shadow-2xl p-6 border border-gray-700">
                        ...
                        {/* Pricing */}
                        <div className="mb-4">
                            <p className="text-xl text-gray-500 line-through mr-2 inline">{courseInfo.oldPrice}</p>
                            <span className="text-4xl font-extrabold text-red-500">{courseInfo.newPrice}</span>
                        </div>
            ...
            <section className="px-12 pt-0 pb-12">
                ...
                {selectedTab === "Overview" && (
                    <div className="text-left text-white max-w-4xl mx-auto">
                        ...
                        <p className="text-[18px] leading-relaxed">{courseInfo.description}</p>
                    </div>
                )}
                ...
                {selectedTab === "Curriculum" && (
                    <div className="text-left text-white max-w-6xl mx-auto">
                        ...
                        {courseInfo.curriculum.map((group, groupIndex) => (
                            <div key={groupIndex} className="bg-[#0B1735] rounded-[15px] shadow-xl mb-6 border border-gray-700 overflow-hidden">
                                ...
                                >
                                    {/* Kolom 1: Group Title */}
                                    <h3 className="text-xl font-bold text-white w-2/5">{group.groupTitle}</h3> 
                                    
                                    {/* Kolom 2: Total Lessons */}
                                    <span className="text-base text-gray-400 w-1/5 text-center">{group.totalLessons} Lessons</span>
                                    
                                    {/* Kolom 3: Total Duration & Toggle Icon */}
                                    <div className="flex items-center justify-end w-2/5 space-x-3">
                                        <span className="text-base text-gray-400">{group.totalDuration}</span>
                                        {/* Tampilkan ikon berdasarkan status grup ini */}
                                        {openGroups[groupIndex] ? <ChevronUp size={20} className="text-[#FFD200]" /> : <ChevronDown size={20} className="text-[#FFD200]" />}
                                    </div>
                                </div>
                                ...
                                {openGroups[groupIndex] && (
                                    <div className="p-0">
                                        {group.items.map((lesson, lessonIndex) => (
                                            <div key={lessonIndex} className={`flex items-center justify-between p-4 border-t border-gray-700 ${lessonIndex % 2 === 0 ? 'bg-[#0B1735]' : 'bg-[#152345]'}`}>
                                                ...
                                                <div className="w-1/5 text-center">
                                                    {lesson.preview ? (
                                                        ...
                                                        <span className="text-xs text-gray-500">Requires Enrollment</span>
                                                    )}
                                                </div>

                                                {/* Kolom 3: Duration */}
                                                <span className="text-sm text-gray-400 w-1/5 text-right">{lesson.duration}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                ...
                {selectedTab === "Instructor" && (
                    <div className="text-center text-white max-w-2xl mx-auto p-10 bg-[#0B1735] rounded-xl shadow-lg">
                        <h2 className="text-3xl font-bold mb-4">Instructor: {courseInfo.instructor.name}</h2>
                        <p className="text-[16px] mb-4">
                            <span className="text-[#FFD200]">Senior Python Instructor</span>
                        </p>
                        <p className="text-[14px] text-[#A9A9A9] leading-relaxed">{courseInfo.instructor.bio}</p>
                    </div>
                )}
            </section>
